...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        if (!player->GetSession()->IsPremium() && !player->IsGameMaster())
        {
			...
			player->GetSession()->SendNotification("Es tut mir leid $N, Du bist kein MMO Elite Mitglied.");
            return true;
        }
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 action)
    {
        ...
        switch(action)
        {
            case 1202: // Heal
                ...
                {
                    player->CLOSE_GOSSIP_MENU();
					player->GetSession()->SendNotification("Es tut mir leid $N, diese Aktion steht zur Zeit nicht zur Verfügung.");
                }
                else
                {
                    player->CLOSE_GOSSIP_MENU();
			...
			case 4026: // Update Skill to Max for Level
				...
				creature->Say("Deine Waffenskills wurden maximiert.", LANG_UNIVERSAL, NULL);
			break;			
			case 4027: // Betrinken
				player->CLOSE_GOSSIP_MENU();
				player->CastSpell(player,46876,false);
				creature->Say("Prost!", LANG_UNIVERSAL, NULL);
			break;
			case 4028: // Ausnüchtern
				player->CLOSE_GOSSIP_MENU();
				player->SetDrunkValue(0, 9);
				creature->Say("Hahahaha! Du scheinst deinen Drink nicht vertragen zu habem!", LANG_UNIVERSAL, NULL);
			break;
			case 7000://Remove Res Sickness
				if (!player->HasAura(15007, ObjectGuid::Empty))
				{
					player->GetSession()->SendNotification("Du hast keine Todesnachwirkungen.");
					OnGossipHello(player, creature);
				}
				creature->CastSpell(player,38588,false); // Healing effect
				player->RemoveAurasDueToSpell(15007, ObjectGuid::Empty);
				player->CLOSE_GOSSIP_MENU();
			break;			
            case 1203: // Morphs
